...
namespace Userdesk\Subscription\Services;
...
class TwoCheckout implements ProcessorContract{
    ...
    /**
     * Return 2Checkout AuthResponse ( without recurrence )
     *
     * @param int $id
     * @param \Userdesk\Subscription\Contracts\Consumer $consumer
     * @param float $total
     ...
     * @return \Illuminate\Http\Response|null
     */
    public function completeWithoutRedirection(int $id, SubscriptionConsumerContract $consumer, float $total = 0, $token = null)
    {
        if (empty($token)) {
            throw new InvalidArgumentException('TwoCheckout generated token must be provided!');
        }
